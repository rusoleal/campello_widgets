...
// ---------------------------------------------------------------------------
// 4d. NESTED — NestedScrollView: a pinned/collapsing header sharing one drag
// gesture with an independently-scrolling body list, via a
// NestedScrollCoordinator (the widget-layer bridge for the NestedScrollView
// mechanism).
// ---------------------------------------------------------------------------
class NestedSection : public cw::StatelessWidget
{
public:
    cw::WidgetRef build(cw::BuildContext& ctx) const override
    {
        ...
        header_content->color = colors.primary;
        header_content->child = cw::mw<cw::Center>(
            cw::mw<cw::Text>("Profile", ts(20.0f, colors.on_primary)));

        auto header = std::make_shared<cw::SliverPersistentHeader>();
        header->min_extent = 60.0f;
        header->max_extent = 200.0f;
        header->child       = header_content;
        ...
        auto nsv = std::make_shared<cw::NestedScrollView>();
        nsv->header = header;
        nsv->body   = { list };
        nsv->inner_physics = std::make_shared<cw::BouncingScrollPhysics>();
