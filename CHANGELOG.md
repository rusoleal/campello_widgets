...
- **Android CI configure failure** — `campello_input` v0.2.1's `android.cmake` requires the AGDK `game-activity` package (`find_package(game-activity REQUIRED)`), which is not installed on standard CI runners. Since `campello_widgets`' Android code does not reference any `campello_input` symbols, the wrapper now creates an `INTERFACE` stub target on Android instead of calling `add_subdirectory`, satisfying the link dependency without needing the full AGDK.
