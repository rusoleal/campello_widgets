...
namespace systems::leal::campello_widgets
{
    ...
    public:
        ...
        void removeErrorListener(uint64_t id)
        {
            std::lock_guard<std::mutex> lock(mutex_);
        ...
        void removeDoneListener(uint64_t id)
        {
            std::lock_guard<std::mutex> lock(mutex_);
            done_listeners_.erase(id);
        }
        ...
        {
            decltype(data_listeners_) copy;
            {
                std::lock_guard<std::mutex> lock(mutex_);
                copy = data_listeners_;
            }
        ...
        {
            decltype(error_listeners_) copy;
            {
                std::lock_guard<std::mutex> lock(mutex_);
                copy = error_listeners_;
            }
        ...
        void _emitDone()
        {
            decltype(done_listeners_) copy;
            {
                std::lock_guard<std::mutex> lock(mutex_);
                copy = done_listeners_;
            }
