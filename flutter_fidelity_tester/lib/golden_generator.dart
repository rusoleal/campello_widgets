...
/// Standalone golden generator that runs with `flutter run` for real text rendering.
/// 
/// Usage: flutter run -d macos lib/golden_generator.dart
/// 
/// This uses real font rendering instead of the blocky Ahem font used in `flutter test`.
...
Future<void> main() async {
  ...
  print('Generating Flutter visual golden files with REAL text rendering...');
  ...
  await runTest(
    ...
    onResult: (success) => success ? generated++ : failed++,
  );
  ...
  // TEXT TESTS - These will now render actual text!
  await runTest(
    name: 'canvas_text_basic',
    size: const Size(kFidelityWidth, kFidelityHeight),
    builder: () => MaterialApp(
      debugShowCheckedModeBanner: false,
      theme: ThemeData(fontFamily: 'Roboto'),
...
Future<void> runTest({
  ...
  try {
    ...
    final outputPath = '${outputDir.path}/$name.png';
    
    await captureWidgetToPng(
      widget: widget,
      size: size,
...
Future<void> captureWidgetToPng({
  ...
  final RenderView renderView = RenderView(
    ...
    child: RenderPositionedBox(
      ...
      child: _RenderWidget(widget: widget, size: size),
    ),
  ...
  // Paint to image
  final offsetLayer = renderView.layer! as OffsetLayer;
  final ui.Image image = await offsetLayer.toImage(Offset.zero & size, pixelRatio: 1.0);
...
// Helper render object to embed a widget in the render tree
class _RenderWidget extends RenderBox {
  final Widget widget;
  final Size size;
  
  _RenderWidget({required this.widget, required this.size});
